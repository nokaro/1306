...
<body style="background-color: #FAF7FB;">
				...
				<form action="./correctPostCtr.do" method="post">
					<div>
						<label>내용: </label> <input type="text" name="content"
							value="${boardVo.content}" required>
					</div>
					<br>
					<div>
						<label>태그: </label> <input type="text" name="tag"
							value="${boardVo.tag}"><br /> <input type="hidden"
							name="pno" value="${boardVo.pno}" />
					</div>
					<input type="submit" class="submit_c" value="수정"> <input
						type="button" class="submit_c" value="삭제"
						onclick="pageMoveDeleteFnc(${boardVo.pno})"> <input
						type="button" class="submit_c" value="취소"
						onclick="pageMoveMyPostFnc(${boardVo.id});">
				</form>
				<!-- 글 전체목록으로 되돌아가기 -->
				<input type="button" class="submit_c" value="글 목록으로"
